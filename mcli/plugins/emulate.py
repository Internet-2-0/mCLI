    ...
    if show_help:
        ...
        if filename is None:
            ...
            try:
                ...
                parsed_output = []
                results = dynamic_results['data'][0]['dynamic_analysis']
                for i, results in enumerate(results):
                    ep_args = results['entry_points'][i]['ep_args']
                    instruction_count = results['entry_points'][i]['instr_count']
                    api_calls = results['entry_points'][i]['apis']
                    # TODO:/
                    # memory_access = results['entry_points'][0]['mem_access']
                    errors = results['entry_points'][i]['error']
                    in_memory_strings = results['strings']['in_memory']
                    run_time = results['emulation_total_runtime']
                    ran_os = results['os_run']
                    start_address = results['entry_points'][i]['start_addr']
                    emulation_breakdown = (
